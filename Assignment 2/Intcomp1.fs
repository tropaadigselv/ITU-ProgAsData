...
let rec assemble (s:sinstr list) : int list =
    match s with
    | SCstI s :: ss -> 0 :: s :: assemble ss
    | SVar s :: ss -> 1 :: s :: assemble ss
    | SAdd :: ss -> 2 :: assemble ss
    | SSub :: ss -> 3 :: assemble ss
    | SMul :: ss -> 4 :: assemble ss
    | SPop :: ss -> 5 :: assemble ss
    | SSwap :: ss -> 6 :: assemble ss
    | [] -> []

let compile e cenv =
    let is = scomp e cenv |> assemble
    intsToFile is "is1.txt"
    
// Exercise 2.5
// See is1.txt - we tested it, and it works :)